...
{
    [TestClass]
    public class DataTypesTests : TestSetup
    {
        // see https://docs.snowflake.com/en/sql-reference/intro-summary-data-types
        private const string DataTypeTestTableName = "TestTableWithAllPossibleDataTypes";
        ...
        public static async Task ClassInitAsync(TestContext _)
        {
            ...
            var sql = $"""
                       CREATE OR REPLACE TABLE "{Connection.DbName}"."{Connection.Schema}"."{DataTypeTestTableName}"
                       (
                           ...
                           Constraint PK_{DataTypeTestTableName} PRIMARY KEY ("Id")
                       );
                       """;
        ...
        [TestMethod]
        public async Task Fetch_Table_Schema_From_Standard_Table()
        {
            var tableSchema = await DatabaseService.GetTableSchemaAsync(DataTypeTestTableName, Connection).ConfigureAwait(false);
            var currentJson = tableSchema.ToJson();

            var currentResult = currentJson.FromJsonStringAs<SnowflakeTableSchema>();

            var expectedSchema = EmbeddedFile.GetFileContentFrom("Fixture.TableSchemaDataTypesResultSnowflake.json").FromJsonStringAs<SnowflakeTableSchema>();
        ...
        public static async Task ClassCleanupAsync()
        {
            ...
            var sql = $"""
                       DROP TABLE IF EXISTS "{Connection.DbName}"."{Connection.Schema}"."{DataTypeTestTableName}";
                       """;
